 ...
 */
const Event = ({id,name,description, days_of_the_week, start_time, end_time, start_date, end_date, location, other_information}) => {
  ...
  return (
    ...
    <Paper  title={name} 
      ...
      <MeetingTimes  days={days_of_the_week} start_time={start_time} end_time={end_time}/>
      <Location location={location} id={id} />
      <Duration start_date={start_date} end_date={end_date} id={id}/>
      {description && <Description description={description} other_information={other_information} />}

      <ProjectButton type='attention' 
        title='Drop Event' 
        style={[styles.margin_medium]}
        onPress={() => {openDropEventModal(id)}}
      />
    </Paper>
</View>
  );
    ...
    <Accordian title="Description"
      ...
      content={
        <View>
          ...
          {/*Description */}
          <Text style={[styles.text_small, styles.padding_medium]}>{description}</Text>

          {/*Other Info*/}
          {other_information.trim().length > 0 &&
          <>
            <Text style={[styles.text_small, styles.padding_medium, styles.bold, ]}>Additional Information</Text>
            <Underline style={[ styles.margin_horizontal_medium]} />
            <Text style={[styles.text_small, styles.padding_medium]}>{other_information}</Text>
          </>
          }
    ...
    <Accordian title="Session Dates"
      ...
      content={
        <View style={[styles.row,styles.padding_horizontal_medium]}>
          ...
          {/*Start Date Column*/}
          <View>
            <Text style={[styles.text_medium, styles.bold,styles.muted_text_2_colour]}>Session Start:</Text>
            <Text style={[styles.text_medium, styles.muted_text_2_colour]}>{start_date}</Text>
          </View>

         {/*End Date Column*/}
          <View>
            <Text style={[styles.text_medium, styles.bold,styles.muted_text_2_colour]}>Session End:</Text>
            <Text style={[styles.text_medium, styles.muted_text_2_colour]}>{end_date}</Text>
          </View>
        </View>
      }
    />
...
const Location = ({id,location}) => {
  ...
  return (
    <View style={[styles.padding_horizontal_medium]}>
        ...
        {/*Section Title */}
        <Text style={[styles.text_medium, styles.muted_text_2_colour, styles.padding_top_small, styles.bold]}>Location:</Text>

        {/*Section Info */}
        <View style={[styles.row,]}>

          {/*Location */}
          <Text style={[styles.text_medium, styles.muted_text_2_colour, styles.padding_top_small]}>{location}</Text>
